...
type VideoInfo struct {
	...
	// Thumbnail of the video author
	AuthorThumbnail string `json:"authorThumbnail"`
	// Duration of the video
	Duration time.Duration
	// View Count of the video
	ViewCount int
	// View Like Count of the video
	LikeCount int
	// View Dislike Count of the video
	DislikeCount int
...
func getVideoInfoFromHTML(id string, html []byte) (*VideoInfo, error) {
	...
	desHtml, errHtml := doc.Find("#eow-description").Html()
	
	if errHtml != nil {
		info.Description = strings.TrimSpace(doc.Find("#eow-description").Text())
	} else {
		info.Description = desHtml
	}
	...
	viewCount, bAttr := doc.Find("meta[itemprop=interactionCount]").Attr("content")
	if bAttr {
		nViewCount, toErr := strconv.Atoi(viewCount)
		
		if toErr != nil {
			log.Debug("Unable to extract view count")
		} else {
			info.ViewCount = nViewCount
		}
	}
	
	authThumb, bThumb := doc.Find("a[class*=\"yt-user-photo\"]").First().Find("img").First().Attr("data-thumb")
	if bThumb {
		info.AuthorThumbnail = authThumb
	}
					
	likeStr := doc.Find("button.like-button-renderer-like-button").First().Find("span.yt-uix-button-content").First().Text()
	dislikeStr := doc.Find("button.like-button-renderer-dislike-button").First().Find("span.yt-uix-button-content").First().Text()

	regexNum := regexp.MustCompile("([^\\d])")
	likeStr = regexNum.ReplaceAllString(likeStr, "")
	dislikeStr = regexNum.ReplaceAllString(dislikeStr, "")
	
	nLikeCount, toLikeErr := strconv.Atoi(likeStr)
		
	if toLikeErr != nil {
		log.Debug("Unable to extract like count")
	} else {
		info.LikeCount = nLikeCount
	}
	
	nDisLikeCount, toDisLikeErr := strconv.Atoi(dislikeStr)
		
	if toDisLikeErr != nil {
		log.Debug("Unable to extract dislike count")
	} else {
		info.DislikeCount = nDisLikeCount
	}
